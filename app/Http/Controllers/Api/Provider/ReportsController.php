...
class ReportsController extends Controller
{
    ...
    public function details(Request $request, $id)
    {
        ...
        $data = DB::table('booking')
            ...
            ->select('users.name', DB::raw('DATE_FORMAT(booking.time_slot, "%M %d") as date_booked'), 'booking_time.time_slot', 'family_plan_type_subcategory.name', 'booking.referal', 'booking.status')
            ->whereBetween('booking.time_slot', [$obj['date_from'][0], $obj['date_to'][0]])
            ->where('booking.clinic_id', $getClinicId[0])
            ->get();
